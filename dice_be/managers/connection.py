...
class ConnectionManager:
    ...
    async def send(self, client: User | PlayerData | ObjectId, data: str | dict | BaseModel):
    ...
    async def broadcast(self, data: str | dict | BaseModel, *, exclude: User = None):
        ...
        await asyncio.gather(
            *(self.send(client_id, data)
                for client_id, _ in self.connections.items() if client_id not in exclude_ids),
            return_exceptions=False
        )

    async def personal_broadcast(self, data_factory: Callable[[PlayerData], str],
